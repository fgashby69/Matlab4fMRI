...
subplot(2,1,2); plot(lags/10,R_B); axis([-200 200 0 1]);
xlabel('time (secs)'); ylabel('R(t)');

figure;
% Plot the hrfs
subplot(3,1,1); plot(t,hrf,t,hrf2); axis([0 25 -.03 .18]);
ylabel('h(t)');

% Plot both BOLD responses
subplot(3,1,2); plot(t,B,t,B2); axis([0 200 -.1 .45]);
ylabel('BOLD Response');

%Compute cross-correlation function & plot
[R12,lags]=xcorr(B,B2,'coeff');
subplot(3,1,3); plot(lags/10,R12); axis([-200 200 0 1]);
xlabel('time (secs)'); ylabel('R(t)');
