...
nTRs=480; beta=2.9; B0=40; delta=.01;
betaV=[beta; B0; delta];
...
subplot(3,1,1); plot(t,box); axis([0 1200 0 1.5]); ylabel('Neural Activation');
...
Bdat=X*betaV+normrnd(0,4,[nTRs,1]);
Bdat_plot=zeros(1,12001);
for i=1:480
    Bdat_plot(i*25)=Bdat(i);
end;
subplot(3,1,2); plot(t,Bdat_plot); axis([0 1200 30 60]); ylabel('BOLD Response');

% Estimate the beta vector & error variance
beta_hat=inv(X'*X)*X'*Bdat
Var_e=(Bdat-X*beta_hat)'*(Bdat-X*beta_hat)/(nTRs-1-length(betaV))
...
hold on; plot(t,Bpred_plot); xlabel('Time (seconds)'); ylabel('BOLD Response');
